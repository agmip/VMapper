    ...
    ChosenEditor.prototype.open = function (keyboardEvent) {
        ...
        if (options.group) {
            if (options.data && Object.keys(options.data).length > 0) {
                for (let groupKey in options.data) {
                    let grp = $('<optgroup label="' + groupKey + '"></optgroup>');
                    for (var i = 0; i < options.data[groupKey].length; i++) {
                        el = $("<option />");
                        el.attr("value", options.data[groupKey][i].id);
                        el.html(options.data[groupKey][i].label);

                        if (originalValue.indexOf(options.data[groupKey][i].id + "") > -1) {
                            el.attr("selected", true);
                        }
                        grp.append(el);
                    }
                    this.$textarea.append(grp);
                }
            }
        } else {
...
//        if (options.group) {
//            $(this).on('click','.group-result',function(){
//                // id of select
